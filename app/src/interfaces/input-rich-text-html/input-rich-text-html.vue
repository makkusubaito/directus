...
const buttonVariants = ["primary", "primaryOutline", "secondary", "secondaryOutline", "accent", "accentOutline", "ghost", "link",]
...
<div :id="field" class="wysiwyg" :class="{ disabled }">
	<editor ref="editorElement" v-model="internalValue" :init="editorOptions" :disabled="editorDisabled"
			...
			<v-card-text>
				<div class="grid">
					<div class="field">
						...
						<v-checkbox v-model="linkSelection.internal" block :label="'internal'"></v-checkbox>
					</div>
					<div v-if="linkSelection.internal" class="field">
						<div class="type-label">{{ 'link to document' }}</div>
						<v-select v-model="linkSelection.internalId" :items="items.items.value.map((item) => {
	return {
		text: item.name,
		value: item.id,
	};
	...
	" show-deselect />
					...
					<div class="field">
						...
						<v-input v-model="linkSelection.displayText"
							:placeholder="t('display_text_placeholder')"></v-input>
					</div>
					...
					<div class="field half-right">
						...
						<v-checkbox v-model="linkSelection.newTab" block
							:label="t(linkSelection.newTab ? 'new_tab' : 'current_tab')"></v-checkbox>
					</div>
					<div class="field half">
						<div class="type-label">{{ 'Button' }}</div>
						<v-checkbox v-model="linkSelection.makeButton" block :label="'Button'"></v-checkbox>
					</div>
					<div v-if="linkSelection.makeButton" class="field half-right">
						<div class="type-label">{{ 'button variant' }}</div>
						<v-select v-model="linkSelection.buttonVariant" :items="buttonVariants" show-deselect />
			...
			<v-card-actions>
				...
				<v-button :disabled="linkSelection.url === null && !linkSelection.internalId && !linkNode"
					@click="saveLink">
					{{ t('save') }}
				</v-button>
			</v-card-actions>
	...
	<v-drawer v-model="codeDrawerOpen" :title="t('wysiwyg_options.source_code')" icon="code" @cancel="closeCodeDrawer">
		<div class="content">
			<interface-input-code :value="code" language="htmlmixed" line-wrapping
				@input="code = $event"></interface-input-code>
		</div>
			...
			<template v-if="imageSelection">
				...
				<div class="grid">
					...
					<div class="field half">
						...
						<v-checkbox v-model="imageSelection.lazy" block
					...
					<div v-if="storageAssetTransform !== 'none' && storageAssetPresets.length > 0" class="field half">
						...
						<v-select v-model="imageSelection.transformationKey"
			...
			<v-tabs-items v-model="openMediaTab">
				<v-tab-item value="video">
					<template v-if="mediaSelection">
						...
						<iframe v-if="mediaSelection.tag === 'iframe'" class="media-preview"
							:src="mediaSelection.previewUrl"></iframe>
						<div class="grid">
							<div class="field">
